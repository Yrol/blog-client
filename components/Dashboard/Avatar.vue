  ...
  <div class="c-avatar" :class="avatarClass">
    <template v-if="type === 'simple'">
      <div class="c-avatar__picture relative">
        <span
          v-if="notify"
          class="absolute top-0 right-0 h-4 w-4 rounded-full bg-white text-xs text-black-3 shadow"
          >{{ notify.text }}</span
        >
        <img
          v-if="img"
        ...
        <span
          v-else
          class="block bg-primary-light rounded-full"
          :class="avatarClass"
        ></span>
      </div>
    </template>
    <template v-else-if="type === 'nav'">
      <div class="c-avatar__picture w-8 mr-3 relative">
        <span
          v-if="notify"
          class="absolute top-0 right-8 h-1 w-1 rounded-full bg-white"
          >{{ notify.text }}</span
        >
